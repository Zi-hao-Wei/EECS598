...
def upfirdn3d(input, kernel, up=1, down=1, pad=(0, 0)):
    ...
    out = upfirdn3d_native(input, kernel, up, up, up, down, down,down, pad[0], pad[1], pad[0], pad[1], pad[0], pad[1])
    # else:
...
def upfirdn3d_native(input, kernel, up_x, up_y, up_z, down_x, down_y, down_z, pad_x0, pad_x1, pad_y0, pad_y1, pad_z0, pad_z1):
    ...
    # Expanding input by interleaving with zeros (upsampling)
    ...
    out = out.reshape([-1, 1, in_h * up_y + pad_y0 + pad_y1, in_w * up_x + pad_x0 + pad_x1, in_d * up_z + pad_z0 + pad_z1])
