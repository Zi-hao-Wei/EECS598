...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        self.sub, self.div = torch.FloatTensor([0.5]).view(1, -1, 1, 1, 1), torch.FloatTensor([0.5]).view(1, -1, 1, 1, 1)
        self.load_network()
        self.print_network()
    ...
    def feed_data(self, data):
        ...
        hr_coord, _ = Util.to_pixel_samples(img_hr,dimension=3,C=1)
        cell = torch.ones_like(hr_coord)
        cell[:, 0] *= 2 / img_hr.shape[-3]
        cell[:, 1] *= 2 / img_hr.shape[-2]
        cell[:, 2] *= 2 / img_hr.shape[-1]
        hr_coord = hr_coord.repeat(img_hr.shape[0], 1, 1, 1)
        cell = cell.repeat(img_hr.shape[0], 1, 1, 1)

#        print("cordinate", hr_coord.shape)
#        print("cell", hr_coord.shape)
