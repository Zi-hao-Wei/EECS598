...
class idm(nn.Module):
    def __init__(self, dim,feat_unfold=False,
        ...
        if self.cell_decode:
            ...
            self.imnet = nn.Sequential(nn.Linear(dim + 3, 256),nn.Linear(256, dim))
    def forward(self, x, shape, scale1, scale2, shift):
        coord = make_coord(shape).repeat(x.shape[0], 1, 1,1).to(dist.get_rank())
    ...
    def query_rgb(self, x_feat, scale1, scale2, shift, coord, cell=None):
        ...
        feat_coord = make_coord(feat.shape[-3:], flatten=False).to(dist.get_rank()) \
            .permute(3, 0, 1, 2) \
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                for vz in vz_lst:
                    ...
                    q_feat = F.grid_sample(feat, coord_.flip(-1).unsqueeze(1), mode='nearest', align_corners=False)[:,:,0,0,:].permute(0,2,1)
                    q_coord = F.grid_sample(feat_coord, coord_.flip(-1).unsqueeze(1), mode='nearest', align_corners=False)[:,:,0,0,:].permute(0,2,1)
                    rel_coord = coord[:,0,:,:] - q_coord
                    for i in range(3):
                        rel_coord[:, :, i] *= feat.shape[-(i+1)]
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        qkv = self.qkv(norm).view(batch, n_head, 3 * head_dim, height, width, depth)
        q, k, v = qkv.chunk(3, dim=2)  # Splitting into query, key, value

        q = q.contiguous().view(batch, n_head, head_dim, -1)
        k = k.contiguous().view(batch, n_head, head_dim, -1)
        v = v.contiguous().view(batch, n_head, head_dim, -1)

        # Calculating attention using einsum for 3D data
         # Attention score calculation
        attn = torch.einsum('bnhd,bnhd->bnh', q, k) / math.sqrt(head_dim)
        attn = torch.softmax(attn, dim=-1)

        # Applying attention to the value tensor
        out = torch.einsum('bnh,bnhc->bnhc', attn, v)
        out = out.reshape(batch, n_head * head_dim, height, width, depth)
...
class UNet(nn.Module):
    def __init__(
        ...
        self.conv_body_first = StyleLayer(1, pre_channel, 3, bias=True, activate=True)
    ...
    def forward(self, x, lr, scaler, time):
        ...
        for i, layer in enumerate(self.ups):
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                x = rearrange(x, 'b (h w d) c -> b c h w d', h=feats[-1].shape[-1], w=feats[-1].shape[-1])
