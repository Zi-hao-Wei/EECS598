...
def transform2tensor(img, min_max=(0, 1)):
    ...
    return img
def make_coord(shape, dimansion=3,ranges=None, flatten=True):
...
def to_pixel_samples(img, dimension=2,C=3):
    """ Convert the image to coord-RGB pairs.
        img: Tensor, (3, H, W)
    """
    coord = make_coord(img.shape[-dimension:],dimension)
    rgb = img.view(C, -1).permute(1, 0)
    return coord, rgb
...
if __name__ == "__main__":
    img = torch.rand(1,1,256,256,10)
    print(img.shape)
    c,r = to_pixel_samples(img,dimension=3,C=1)
